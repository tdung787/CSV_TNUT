...
<head>
    ...
    <link rel="shortcut icon" href="{{ asset('website/images/logo_htx_rm_bg.png') }}" type="image/png">
    <title>
        HTX
    </title>
    ...
    <meta property="og:image" content="{{ asset('website/images/logo_htx_rm_bg.png') }}">
    <meta property="og:image:secure_url" content="{{ asset('website/images/logo_htx_rm_bg.png') }}">
    ...
    <link rel="stylesheet" href="{{ asset('website/css/swatch_style.scss.css') }}" as="style"
        onload="this.onload=null;this.rel='stylesheet'">
    <noscript>
        <link rel="stylesheet" href="{{ asset('website/css/swatch_style.scss.css') }}">
    </noscript>

    <link rel="stylesheet" href="{{ asset('website/css/mew_noti.scss.css') }}" as="style"
        onload="this.onload=null;this.rel='stylesheet'">
    <noscript>
    ...
    <link href="{{ asset('website/css/collection_style.scss.css') }}" rel="stylesheet" type="text/css"
        media="all">
...
<body id="body_m">
    <div class="page-body">
        ...
        <div class="fix-phone p-1 p-lg-2 pt-lg-3 pb-lg-3 d-none d-lg-block">
            <a href="tel:0339902798" title="Hoiline"
        ...
        <div class="fix-phone-mb position-fixed d-flex d-lg-none flex-column bg-white p-2 px-lg-3 rounded-10"
            ...
            <div class="item phone">
                <a href="tel:0339902798" rel="nofollow" class="align-items-center border-bottom d-flex gap_5">
                    <span class="img">
                        <img alt="Hotline"
                            src="https://theme.hstatic.net/200000830051/1001188866/14/hotline.png?v=502">
                    </span>
                    <div class="detail">0339902798</div>
                </a>
            </div>
            <div class="item zalo">
            ...
            <div class="item zalo">
                <a target="_blank" href="https://zalo.me/+1 800 1231234" rel="nofollow"
        ...
        <div class="d-flex d-lg-none mew_mobi_bar position-fixed pt-2">
                ...
                <span class="d-inline-block m-auto w-22">
                    <picture class="position-relative m-0 ratio1by1 d-block aspect">
                        <img src="https://file.hstatic.net/200000830051/file/flash_8770ee4267b44a06b795c1eaca7cb87c.gif"
                            alt="Flash Sale" class="d-block img position-absolute">
                    </picture>
                </span>
                <span class="tit d-block font-weight-bold text-center pr-1 pr-sm-2 pl-1 pl-sm-2 pt-1">Flash
                    Sale</span>
            </a>
            ...
            <a href="javascript:;" title="Tư vấn"
        ...
        <div class="toast-container">
            @if (session('success'))
                <div class="toast success active">
                    <p class="t-title">
                        <span class="t-icon-success"></span>Thành công
                    </p>
                    <p class="t-close" onclick="closeToast(this)">×</p>
                    <p class="t-text">{{ session('success') }}</p>
                </div>
            @endif
            @if (session('error'))
                <div class="toast error active">
                    <p class="t-title">
                        <span class="t-icon-error"></span>Lỗi
                    </p>
                    <p class="t-close" onclick="closeToast(this)">×</p>
                    <p class="t-text">{{ session('error') }}</p>
                </div>
            @endif
        </div>
        <script>
        ...
        <script>
            window.iconChatbotUrl = "{{ asset('website/images/icon_chatbot.png') }}";
        </script>
        <script src="{{ asset('website/images/chatbox.js') }}"></script>
        <script>
        ...
        @php
            ...
            $featuredProducts = \App\Models\Admin\Product::latest()->limit(8)->get();
        @endphp
        <div id="popsale" class="jas-sale-pop flex pf middle-xs rounded-10 bg-white box_shadow position-fixed">
            @foreach ($featuredProducts as $product)
                ...
                <div class="jas-sale-pop-content">
                    ...
                    <span class="jas-sale-pop-timeago small text-muted">
                        Vừa được mua cách đây {{ $product->created_at->diffForHumans() }}
                    </span>
                </div>
        ...
        <script>
            const popsale_cate = [
                @foreach ($featuredProducts as $product)
                    {
                        "img_url": "{{ asset('storage/' . ($product->images->first() ? 'product/images/' . $product->id . '/' . $product->images->first()->file_name : 'default-image.jpg')) }}",
                        "title": "{{ $product->name }}",
                        "url": "{{ route('web.product.show_only_product', ['category' => $product->category->slug, 'product' => $product->slug]) }}",
                    },
                @endforeach
            ];
